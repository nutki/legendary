"use strict";
/* global isHero, revealable, yourHeroes, Color, owned, playerState, handOrDiscard, isWound, gameState, HQCards */
addTemplates("VILLAINS", "Legendary", [
    ...
    { name: "Enemies of Asgard", cards: [
            ...
            [2, makeVillainCard("Enemies of Asgard", "Ymir, Frost Giant King", 6, 4, {
                    ...
                    fight: ev => selectPlayerEv(ev, psel => selectObjectsOptEv(ev, "KO any Wounds", 999, handOrDiscard(psel).limit(isWound), sel => KOEv(ev, sel), psel)),
                })],
        ] },
    { name: "HYDRA", cards: [
            ...
            [3, makeVillainCard("HYDRA", "HYDRA Kidnappers", 3, 1, {
                    fight: ev => chooseMayEv(ev, "Gain S.H.I.E.L.D. Officer", ev => { if (gameState.officer.top)
                        gainEv(ev, gameState.officer.top); }),
                })],
            ...
            [1, makeVillainCard("HYDRA", "Viper", 5, 3, {
                    fight: ev => eachPlayer(p => { if (p.victory.count(isGroup("HYDRA")) === 0)
                        gainWoundEv(ev, p); }),
                    escape: ev => eachPlayer(p => { if (p.victory.count(isGroup("HYDRA")) === 0)
                        gainWoundEv(ev, p); }),
                })],
        ] },
    { name: "Masters of Evil", cards: [
            ...
            [2, makeVillainCard("Masters of Evil", "Whirlwind", 4, 2, {
                    fight: ev => { if (ev.where === "ROOFTOPS" || ev.where === "BRIDGE")
                        selectObjectsEv(ev, "Choose Heroes to KO", 2, yourHeroes(), sel => KOEv(ev, sel)); },
                })],
        ] },
    { name: "Radiation", cards: [
            ...
            [2, makeVillainCard("Radiation", "Abomination", 5, 3, {
                    fight: ev => { if (ev.where === "STREETS" || ev.where === "BRIDGE")
                        rescueEv(ev, 3); },
                })],
    ...
    { name: "Spider-Foes", cards: [
            ...
            [2, makeVillainCard("Spider-Foes", "The Lizard", 3, 2, {
                    fight: ev => { if (ev.where === "SEWERS")
                        eachOtherPlayerVM(p => gainWoundEv(ev, p)); },
                })],
