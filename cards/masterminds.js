...
makeMastermindCard("Dr. Doom", 9, 5, "Doombot Legion", ev => {
  ...
  eachPlayer(p => { if (p.hand.size === 6) revealOrEv(ev, Color.BLACK, ev => { pickTopDeckEv(ev, p); pickTopDeckEv(ev, p); }, p); });
}, [
  // You may recruit a [Tech] or [Ranged] Hero from the HQ for free.
  [ "Dark Technology", ev => {
    selectCardEv(ev, HQCards().limit(Color.BLACK | Color.BLUE), ev => recruitForFreeEv(ev, ev.selected));
  } ],
...
// Each player reveals a [Strength] Hero or gains a Wound
  ...
  [ "Cruel Ruler", ev => selectCardEv(ev, CityCards().limit(isVillain), ev => defeatEv(ev, ev.selected)) ],
  ...
  [ "Vanishing Illusions", ev => eachOtherPlayerVM(p => selectCardEv(ev, p.victory.limit(isVillain), ev => KOEv(ev, ev.selected), p)) ],
  // Each other player KOs two Bystanders from their Victory Pile.
  [ "Whispers and Lies", ev => eachOtherPlayerVM(p => selectCardsNEv(ev, 2, p.victory.limit(isBystander), ev => KOEv(ev, ev.selected), p)) ],
]),
makeMastermindCard("Magneto", 8, 5, "Brotherhood", ev => {
// Each player reveals an X-Men Hero or discards down to four cards.
  eachPlayer(p => revealOrEv(ev, 'X-Men', ev => selectCardsNEv(ev, p.hand.size - 4, p.hand, ev => discardEv(ev, ev.selected), p), p));
}, [
  // Recruit an X-Men Hero from the HQ for free.
  [ "Bitter Captor", ev => selectCardEv(ev, HQCards().limit('X-Men'), ev => recruitForFreeEv(ev, ev.selected)) ],
  ...
  [ "Electromagnetic Bubble", ev => {
    selectCardEv(ev, yourHeroes().limit('X-Men'), ev => addTurnTrigger("ENDOFTURN", u, ev => moveCardEv(ev, ev.selected, playerState.hand))); // TODO move instance instead
  } ],
  // For each of your X-Men Heroes, rescue a Bystander.
  [ "Xavier's Nemesis", ev => {
    rescueEv(ev, youdHeroes().limit("X-Men").size);
  } ],
]),
makeMastermindCard("Red Skull", 7, 5, "HYDRA", ev => {
// Each player KOs a Hero from their hand.
  eachPlayer(p => selectCardEv(ev, p.hand.limit(isHero), ev => KOEv(ev, ev.selected), p));
}, [
  // You get +4 Recruit.
