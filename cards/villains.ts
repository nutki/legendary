...
// VP: 3
  [ 1, makeVillainCard("Kree Starforce", "Korath the Pursuer", 5, 3, {
    ambush: ev => {
      eachPlayer(p => chooseMayEv(ev, "Draw a card", () => drawEv(ev, 1, p), p));
      cont(ev, () => attachShardEv(ev, ev.source, pastEvents('DRAW').count(e => ancestorEvents(e).includes(ev))));
    },
...
// VP: 5
  [ 1, makeVillainCard("Dark Avengers", "Sentry", 7, 5, {
    ...
    fight: ev => isLocation(ev.where, "BANK", "STREETS") && selectObjectsUpToEv(ev, "Choose up to two cards to KO", 2, playerState.discard.deck, c => KOEv(ev, c)),
...
// VP: 2
  [ 1, makeVillainCard("Weapon Plus", "Huntsman (Weapon XII)", 2, 2, {
    ...
    fight: ev => {
      ...
      ev.source.attached('HUNTSMAN_CAPTURED').size && enterCityEv(ev, ev.source, u, u, true);
    },
    varDefense: weaponXSequenceVarDefense(1),
  })],
// {WEAPON X SEQUENCE}
...
// VP: 6
  [ 1, makeVillainCard("Weapon Plus", "Ultimaton (Weapon XV)", 4, 6, {
    ...
    fight: ev => eachPlayer(p => selectCardAndKOEv(ev, yourHeroes(p).limit(Color.GRAY), p)),
    escape: ev => eachPlayer(p => selectCardAndKOEv(ev, yourHeroes(p).limit(isNonGrayHero), p)),
