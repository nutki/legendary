...
{ card: [ 3, makeBystanderCard("Paramedic", ev => selectCardOptEv(ev, gameState.players.map(p => p.discard).reduce((a, d) => a.concat(d), playerState.hand).limit(isWound), c => KOEv(ev, c))) ] }, // TODO concat
// RESCUE: you may KO one of your Heroes or a Hero from your discard pile.
{ card: [ 4, makeBystanderCard("Radiation Scientist", ev => selectCardOptEv(ev, yourHeroes().concat(playerState.discard).limit(isHero), c => KOEv(ev, c))) ] }, //TODO join
]);