"use strict";
/* global Color, gameState, isVillain, isBystander, HQCards, isHero, twistTemplate, villainIn, evilWinsEv, isEnemy, CityCards, rescueBystander, turnState, eachPlayer, Card, playerState, villainOrMastermind, villainDrawEv */
addTemplates("SCHEMES", "Legendary", [
    ...
    makeSchemeCard("Midtown Bank Robbery", { twists: 8, vd_bystanders: 12 }, ev => {
        ...
        event: "ESCAPE",
        after: ev => { if (gameState.escaped.count(isBystander) >= 8)
            evilWinsEv(ev); },
    }, () => {
    ...
    makeSchemeCard("Replace Earth's Leaders with Killbots", { twists: 5, vd_bytstanders: 18 }, ev => {
        ...
        event: "ESCAPE",
        after: ev => { if (gameState.escaped.count(isBystander) >= 5)
            evilWinsEv(ev); },
    }, function () {
    ...
    makeSchemeCard("Secret Invasion of the Skrull Shapeshifters", { twists: 8, heroes: 6, required: { villains: "Skrulls" } }, ev => {
        ...
        event: "ESCAPE",
        after: ev => { if (gameState.escaped.count(isHero) >= 6)
            evilWinsEv(ev); },
    }, function () {
        // TODO implement these mods
...
addTemplates("SCHEMES", "Dark City", [
    ...
    makeSchemeCard("Capture Baby Hope", { twists: 8 }, ev => {
        ...
        if (a instanceof Card && isVillain(a)) {
            ...
            cont(ev, () => { if (gameState.mastermind.attached("TWIST").size >= 3)
                evilWinsEv(ev); });
        }
        else
            CityCards().limit(isVillain).withLast(v => captureEv(ev, v, hope));
    }, [], () => {
    ...
    makeSchemeCard("Detonate the Helicarrier", { twists: 8, heroes: 6 }, ev => {
        ...
        repeat(ev.nr, () => cont(ev, () => HQCards().limit(isHero).withFist(c => KOEv(ev, c))));
            ...
            before: ev => {
                ...
                if (ev.hq.explosion === 6)
                    ev.hq.isHQ = false;
                if (!gameState.hq.has(l => l.isHQ))
                    evilWinsEv(ev);
            },
    ...
    makeSchemeCard("Massive Earthquake Generator", { twists: 8 }, ev => {
        ...
        eachPlayer(p => revealOrEv(ev, Color.STRENGTH, ev => lookAtDeckEv(ev, 1, () => p.revealed.withLast(c => KOEv(ev, c)), p), p));
            ...
            event: "KO",
            after: ev => { if (gameState.ko.count(c => isHero(c) && !isColor(Color.GRAY)(c)) >= 3 * gameState.players.size)
                evilWinsEv(ev); },
        }]),
    ...
    makeSchemeCard("Organized Crime Wave", { twists: 8 }, ev => {
        ...
        event: "ESCAPE",
        after: ev => { if (gameState.escaped.count(gameState.isGoon) >= 5)
            evilWinsEv(ev); },
    }, () => {
    ...
    makeSchemeCard("Save Humanity", { twists: 8 }, ev => {
        ...
        eachPlayer(p => revealOrEv(ev, Color.INSTINCT, ev => selectCardAndKOEv(ev, p.victory.limit(isBystander), p), p));
            ...
            event: "MOVECARD",
            after: ev => { if (gameState.ko.count(isBystander) + gameState.escaped.count(isBystander) >= 4 * gameState.players.size)
                evilWinsEv(ev); }
        }], () => {
        repeat(gameState.players.size === 1 ? 12 : 24, () => moveCard(gameState.bystanders.top, gameState.herodeck));
        gameState.specialActions = (ev) => {
            if (turnState.recruit < 2)
                return [];
    ...
    makeSchemeCard("Steal the Weaponized Plutonium", { twists: 8, vd_villain: [2, 3, 4, 4, 5] }, ev => {
        ...
        villainDrawEv(ev);
            ...
            event: "ESCAPE",
            after: ev => { if (gameState.escaped.count(c => c.cardType === "SCHEME TWIST") >= 4)
                evilWinsEv(ev); },
        }, {
            event: "DEFEAT",
            after: ev => {
    ...
    makeSchemeCard("X-Cutioner's Song", { twists: 8, vd_bystanders: 0, heroes: [4, 6, 6, 6, 7] }, ev => {
        ...
        villainDrawEv(ev);
            ...
            event: "ESCAPE",
            after: ev => { if (gameState.escaped.count(c => isHero(c) && !isColor(Color.GRAY)(c)) >= 9)
                evilWinsEv(ev); },
        }], () => {
