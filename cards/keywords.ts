...
function focusActionEv(ev: Ev, cost: ActionCost, effect: (ev: Ev) => void, limit?: number) {
  ...
  return new Ev(ev, 'FOCUS', { what, func, cost, desc });
}
...
function isFortifying(c: Card, d: Deck | Card): boolean {
  if (d instanceof Card) return d.location ? isFortifying(c, d.location) : false;
...
function wakingNightmareOptEv(ev: Ev, effect?: (c: Card) => void, p: Player = playerState) {
...
function isPreying(c: Card) {
  return gameState.players.some(p => p.deck.attachedDeck('PREYING') === c.location);
}
function finishThePreyEv(ev: Ev, card: Card) {
  eachPlayer(p => {
    if(card.location === p.deck.attachedDeck('PREYING')) {
...
function preyEv(ev: Ev, handScore: (p: Player) => number, immediateEffect?: (p: Player) => void, card: Card = ev.source) {
  choosePlayerLimitedEv(ev, p => {
    attachCardEv(ev, card, p.deck, 'PREYING');
    const setupTrigger = () => {
