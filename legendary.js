...
function makeCardInPlay(c, where, bottom) {
    if (c.ctype === "P")
        throw TypeError("need card template");
...
function makeCardCopy(c) {
    if (c.ctype !== 'P')
        throw TypeError("need card in play");
...
Deck.prototype = {
    ...
    addNewCard: function (c, n) { let r = undefined; for (let i = 0; i < (n || 1); i++)
        r = makeCardInPlay(c, this); return r; },
    ...
    remove: function (c) { let p = this.deck.indexOf(c); if (p >= 0)
        this.deck.splice(p, 1); return p >= 0; },
    ...
    withTop: function (f) { if (this.size !== 0)
        f(this.top); },
    withFirst: function (f) { if (this.size !== 0)
        f(this.first); },
    withLast: function (f) { if (this.size !== 0)
        f(this.last); },
    attachedDeck: function (name) { return attachedDeck(name, this); },
    attached: function (name) { return attachedCards(name, this); },
    deckList: [],
...
Array.prototype.withFirst = function (f) { if (this.size !== 0)
    f(this.first); };
Array.prototype.withLast = function (f) { if (this.size !== 0)
    f(this.last); };
function repeat(n, f) { for (let i = 0; i < n; i++)
    f(i); }
let Ev = function (ev, type, params) {
    ...
    if (!this.ui && (!this.func || typeof this.func !== "function"))
        throw TypeError("No function in event");
};
Ev.prototype = {
    getSource: function () {
...
function makeSchemeCard(name, counts, effect, triggers, initfunc) {
    ...
    c.twist = effect;
    if (triggers)
        c.triggers = triggers instanceof Array ? triggers : [triggers];
    if (initfunc)
        c.init = initfunc;
    return c;
}
...
function initGameState() {
        ...
        triggers: [
            {
                ...
                after: ev => { if (!ev.parent.from.size)
                    runOutEv(ev, ev.parent.from.id); },
            },
            {
                event: "MOVECARD",
                match: function (ev) { return ev.from.isHQ; },
                after: function (ev) { if (gameState.herodeck.size)
                    moveCardEv(ev, gameState.herodeck.top, ev.parent.from); },
            },
            {
                event: "MOVECARD",
                match: function (ev) { return ev.to.isCity && ev.to.size; },
                before: function (ev) { let to = ev.parent.to; if (to.next)
                    moveCardEv(ev, to.top, to.next);
                else
                    villainEscapeEv(ev, to.top); },
            },
            {
                event: "DEFEAT",
                match: function (ev) { return ev.what.location === gameState.mastermind; },
                after: function (ev) { if (!gameState.mastermind.has(c => !c.tacticsTemplates || c.attached("TACTICS").size))
                    gameOverEv(ev, "WIN"); },
            },
            {
                event: "CLEANUP",
                match: () => true,
                after: ev => { if (!gameState.villaindeck.size || !gameState.herodeck.size)
                    gameOverEv(ev, "DRAW"); },
            },
        ],
        endDrawAmount: 6,
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (i)
            gameState.city[i].next = gameState.city[i - 1];
    }
    let gameSetup = {
        /* S01M01
    ...
    if (gameSetup.withOfficers)
        gameState.officer.addNewCard(officerTemplate, 30);
    if (gameSetup.withWounds)
    ...
    for (let i = 0; i < gameState.endDrawAmount; i++)
        gameState.players.forEach(p => moveCard(p.deck.top, p.hand));
    if (gameState.scheme.top.init)
        gameState.scheme.top.init();
}
...
function HQCardsHighestCost() {
    let all = HQCards();
    let maxCost = 0;
    all.forEach(function (c) { if (c.cost > maxCost)
        maxCost = c.cost; });
...
function eachOtherPlayer(f) { let r = gameState.players.filter(function (e) { return e !== playerState; }); if (f)
    r.forEach(f); return r; }
function eachOtherPlayerVM(f) { return gameState.advancedSolo ? eachPlayer(f) : eachOtherPlayer(f); }
function eachPlayer(f) { if (f)
    gameState.players.forEach(f); return gameState.players; }
function eachPlayerEv(ev, f) { eachPlayer(p => pushEv(ev, "EFFECT", { who: p, func: f })); }
function revealable(who = playerState) {
    // TODO: also artifacts and maybe MOoT
    if (who !== playerState)
        return who.hand.deck;
...
function canFight(c) {
    if (c.fightCond && !c.fightCond(c))
        return false;
    let a = turnState.attack;
    if (c.bribe || turnState.attackWithRecruit)
        a += turnState.recruit;
    return a >= c.defense;
}
function canHeal(c) {
...
function getActions(ev) {
    ...
    if (gameState.specialActions)
        p = p.concat(gameState.specialActions(ev));
    p = p.concat(new Ev(ev, "ENDOFTURN", { confirm: p.length > 0, func: ev => ev.parent.endofturn = true }));
    return p;
}
function addAttackEvent(ev, c) { pushEv(ev, "ADDATTACK", { func: ev => turnState.attack += ev.amount, amount: c }); }
function addRecruitEvent(ev, c) { pushEv(ev, "ADDRECRUIT", { func: ev => { turnState.recruit += ev.amount; turnState.totalRecruit += ev.amount; }, amount: c }); }
...
function swapCardsEv(ev, where1, where2) {
    cont(ev, () => {
        ...
        if (what1)
            moveCard(what1, where2);
        if (what2)
            moveCard(what2, where1);
    });
}
...
function discardEv(ev, card) { pushEv(ev, "DISCARD", { what: card, func: ev => moveCardEv(ev, ev.what, ev.what.location.owner.discard) }); }
...
function KOEv(ev, card) { pushEv(ev, "KO", { what: card, func: ev => moveCardEv(ev, ev.what, gameState.ko) }); }
...
function runOutEv(ev, deck) { pushEv(ev, "RUNOUT", { what: deck, func: () => { } }); }
function captureEv(ev, villain, what) {
    if (what)
        pushEv(ev, "CAPTURE", { func: ev => attachCardEv(ev, ev.what, ev.villain, "CAPTURED"), what: what, villain: villain });
    else
        cont(ev, () => {
            if (gameState.bystanders.top)
                pushEv(ev, "CAPTURE", { func: ev => attachCardEv(ev, ev.what, ev.villain, "CAPTURED"), what: gameState.bystanders.top, villain: villain });
        });
}
function gainWoundEv(ev, who) {
    who = who || playerState;
    cont(ev, () => {
        if (gameState.wounds.top)
            gainEv(ev, gameState.wounds.top, who);
    });
}
function cont(ev, func) { pushEv(ev, "EFFECT", func); }
function pushEv(ev, name, params) { let nev = new Ev(ev, name, params); pushEvents(nev); return nev; }
function selectObjectsMinMaxEv(ev, desc, min, max, objects, effect1, effect0, simple, who) {
    if (objects instanceof Deck)
        objects = objects.deck;
    who = who || playerState;
    if (objects.length === 0) {
        if (effect0)
            cont(ev, () => effect0());
        ...
        if (effect1)
            cont(ev, () => objects.forEach(effect1));
    }
    else {
...
function chooseOneEv(ev, desc, ...a) {
    let options = [];
...
function selectPlayerEv(ev, f, who) {
    if (gameState.players.length === 1) {
        ...
        selectObjectEv(ev, "Choose a Player", gameState.players, f, who); // TODO multiplayer
}
...
function lookAtDeckEv(ev, amount, action, who, agent) {
    ...
    for (let i = 0; i < amount; i++)
        cont(ev, ev => revealOne(ev, who));
    cont(ev, action);
    let cleanupRevealed = () => {
        if (who.revealed.size === 0)
            return;
        if (who.revealed.size === 1)
            moveCardEv(ev, who.revealed.top, who.deck);
        else
            selectCardEv(ev, "Choose a card to put back", who.revealed, sel => { moveCardEv(ev, sel, who.deck); cleanupRevealed(); }, agent);
    };
...
function playCardEffects(ev, card) {
    ...
    pushEv(ev, "PLAYCARDEFFECTS", { source: card, func: ev => {
            if (card.playCostType === "DISCARD")
                pickDiscardEv(ev);
            if (card.attack)
                addAttackEvent(ev, card.attack);
            if (card.recruit)
                addRecruitEvent(ev, card.recruit);
...
function playTwistEv(ev, what) { pushEv(ev, "TWIST", { func: playTwist, what: what }); }
function playTwist(ev) {
    let e = pushEv(ev, "EFFECT", { source: gameState.scheme.top, func: gameState.scheme.top.twist, nr: ++gameState.twistCount, twist: ev.what });
    cont(ev, () => {
        if (gameState.players.length === 1) {
            if (gameState.advancedSolo)
                selectCardEv(ev, "Choose a card to put on the bottom of the Hero deck", HQCards().limit(c => c.cost <= 6), sel => moveCardEv(ev, sel, gameState.herodeck, true));
            else
...
function villainDrawEv(ev) { pushEv(ev, "VILLAINDRAW", villainDraw); }
function villainDraw(ev) {
    ...
    if (!c) {
        ...
        if (c.ambush)
            pushEv(ev, "EFFECT", { source: c, func: c.ambush });
    }
...
function villainEscapeEv(ev, what) { pushEv(ev, "ESCAPE", { what, func: villainEscape }); }
function villainEscape(ev) {
    ...
    if (b.has(isBystander))
        eachPlayer(p => pickDiscardEv(ev, p));
    moveCardEv(ev, c, gameState.escaped);
    selectCardAndKOEv(ev, HQCards().limit(c => c.cost <= 6));
    if (c.escape)
        pushEv(ev, "EFFECT", { source: c, func: c.escape });
}
...
function villainDefeat(ev) {
    ...
    if (c.fight)
        pushEv(ev, "EFFECT", { source: c, func: c.fight });
}
function rescueEv(ev, what) {
    if (what && typeof what !== "number")
        pushEv(ev, "RESCUE", { func: rescueBystander, what: what });
    else
        for (let i = 0; i < (what || 1); i++)
            cont(ev, () => {
                if (gameState.bystanders.top)
                    pushEv(ev, "RESCUE", { func: rescueBystander, what: gameState.bystanders.top });
            });
}
function rescueBystander(ev) {
    ...
    moveCardEv(ev, c, playerState.victory);
    if (c.rescue)
        pushEv(ev, "EFFECT", { source: c, func: c.rescue });
}
...
function findTriggers(ev) {
    ...
    let checkTrigger = source => t => {
        if (t.event === ev.type && t.match(ev, source))
            triggers.push({ trigger: t, source: source });
    };
    let checkCardTrigger = c => {
        if (c.trigger)
            checkTrigger(c)(c.trigger);
    };
...
function addTriggers(ev) {
    ...
    triggers.forEach(function (t) {
        if (t.trigger.before)
            newev.push(t.state = new Ev(ev, "EFFECT", { func: t.trigger.before, source: t.source }));
    });
    newev.push(ev);
    triggers.forEach(function (t) {
        if (t.trigger.after)
    ...
    triggers.forEach(function (t) {
        if (t.trigger.replace)
            newev = [new Ev(ev, "EFFECT", { func: t.trigger.replace, what: newev, source: t.source })];
    });
    return newev;
}
...
function cardImageName(card) {
    if (card.cardType === "HERO")
        return imageName("heroes", card.heroName, card.cardName);
    if (card.cardType === "VILLAIN" && card.villainGroup === undefined)
        return imageName("henchmen", card.cardName);
    if (card.cardType === "VILLAIN")
        return imageName("villains", card.villainGroup, card.cardName);
    if (card.cardType === "MASTERMIND")
        return imageName("masterminds", card.cardName);
    if (card.cardType === "TACTICS")
        return imageName("masterminds", card.mastermind.cardName, card.cardName);
    if (card.cardType === "SCHEME")
        return imageName("schemes", card.cardName);
    return imageName("", card.cardType);
}
function makeDisplayCard(c) {
    let res = `<span class="card" id="${c.id}" >${c.id}</span>`;
    if (c._attached)
        for (let i in c._attached)
            if (c._attached[i].size) {
...
function getEventName(ev) {
    if (ev.type === "ENDOFTURN")
        return "End Turn";
    if (ev.name)
        return ev.name;
    if (ev.what)
        return `${ev.type} ${ev.what}`;
    ...
    ({
        ...
        "SELECTOBJECTS": function () {
            ...
            extraActions.push({ name: "Confirm", func: () => {
                    ...
                    if (num < ev.min || num > ev.max) {
                        console.log(`${num} not in ${ev.min}-${ev.max}`);
                        return;
                    }
