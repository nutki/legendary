...
function hasFlag(flag: 'N' | 'D' | 'G' | 'F' | 'T' | 'R' | 'A' | '4') { return (c: Card) => c.flags && c.flags.includes(flag); }
...
function playCard2(ev: Ev) {
  ...
  pushEv(ev, "PLAYCARDEFFECTS", { source: card, func: ev => {
    ...
    cont(ev, () => turnState.cardsPlayed.push(card.instance?.copyPasteCard ? Object.assign(Object.create(Object.getPrototypeOf(card)), card) : card));
...
function villainFight(ev: Ev): void {
  const c = ev.what;
  textLog.log(`Fought `, c.instance === c.backSide ? c : c.instance || c);
